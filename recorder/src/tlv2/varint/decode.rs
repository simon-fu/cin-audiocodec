...
use super::MAX_VARINT32_ENCODED_LEN;
use super::MAX_VARINT_ENCODED_LEN;
...
#[inline]
fn decode_varint_full<D: DecodeVarint>(rem: &[u8]) -> Result<Option<(D, usize)>, ()> {
    let mut r: u64 = 0;
...
#[inline]
fn decode_varint_impl<D: DecodeVarint>(buf: &[u8]) -> Result<Option<(D, usize)>, ()> {
...
#[inline]
pub fn decode_varint64(buf: &[u8]) -> Result<Option<(u64, usize)>, ()> {
    decode_varint_impl(buf)
}

/// Try decode a varint. Return `None` if the buffer does not contain complete varint.
#[inline]
pub fn decode_varint32(buf: &[u8]) -> Result<Option<(u32, usize)>, ()> {
    decode_varint_impl(buf)
}
